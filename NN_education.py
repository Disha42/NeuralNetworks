...
with open(sys.argv[2], "r") as label_file:
        ...
        for line in label_file:
            train_labels.append(float(line.rstrip()))
        ...
        train_labels = (train_labels-minVal)/rangeVal
        #print(train_labels)
...
def feedForwardGrad(weightsVector0Grad,weightsVector1Grad):
    ...
    errors = 0
    hiddenOutputGrad =np.zeros((400,3))
    finalOutputGrad =np.zeros(400)
    oneColumn = np.ones(400).reshape(400,1)
...
def backPropGrad(weightsVector0Grad,weightsVector1Grad, target, predicted,x,hiddenOutputGrad):
    ...
    delWeight0 = 0.025 * np.dot(del0.T,hiddenOutputGrad)
    ...
    delWeight1 = 0.025 * (np.dot(del1.T, x))
